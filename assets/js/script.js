jQuery(function ($) {
  ...
  $(document).on('focusout', '.quantity_input', function () {
    var a = $(this).val();
    (isNaN(parseFloat(a)) && !isFinite(a)) || 0 == parseInt(a) || '' == a
      ? $(this).val(1)
      : parseInt(a) < 0
      ? $(this).val(parseInt(a) - 2 * parseInt(a))
      : $(this).val(parseInt(a));
  }),
    $(document).on('click', '.quantity_up', function () {
      var a = $(this).parent().find('.quantity_input');
      isNaN(parseFloat(a.val())) || !isFinite(a.val()) || a.attr('disabled')
        ? a.val(1)
        : a.val(parseInt(a.val()) + 1);
    }),
    $(document).on('click', '.quantity_down', function () {
      var a = $(this).parent().find('.quantity_input');
      !isNaN(parseFloat(a.val())) &&
      isFinite(a.val()) &&
      a.val() > 1 &&
      !a.attr('disabled')
        ? a.val(parseInt(a.val()) - 1)
  ...
  $(document).ready(function () {
    if ($('.rte').length) {
      $('.rte iframe[src *= youtube]').wrap(
        '<div class="rte_youtube_wrapper"></div>'
      );
    }
  });
  ...
  $.fn.formValidation = function () {
    ...
    ).after('<p class="alert-inline" style="display: none;"></p>');
  ...
  jQuery(function ($) {
    $(document.body).on('click', '.quick_view_btn', function (e) {
      ...
      $.getJSON($(this).attr('href') + '.js', function (product) {
        ...
        $('#quick_view__name').html(
          '<a href="' + product.url + '">' + productTitle + '</a>'
        );

        // PRODUCT DESCRIPTION
        var productDescription =
          $.trim(product.description).substring(0, 150) + '...';
        ...
        $('#quantity').on('focusout', function () {
          ...
          $('#quantity_up').on('click', function () {
            var t = $('#quantity').val();
            $('#quantity').val(
              !isNaN(parseFloat(t)) && isFinite(t) ? parseInt(t) + 1 : 1
            );
          }),
          $('#quantity_down').on('click', function () {
            var t = $('#quantity').val();
            $('#quantity').val(
              !isNaN(parseFloat(t)) && isFinite(t) && t > 1
                ? parseInt(t) - 1
                : 1
            );
          });
        ...
        $.getScript(
          '//cdn.shopify.com/shopifycloud/shopify/assets/themes_support/option_selection-fe6b72c2bbdd3369ac0bfefe8648e3c889efca213baefd4cfb0dd9363563831f.js',
          function () {
            ...
            preloadImages(product.images, function () {
              // APPENDING BIG IMAGE
              var bigImgUrl = product.images[0]
                .replace('.png', '_377x380_crop_center.png')
                .replace('.jpg', '_377x380_crop_center.jpg');
              $('#product_quick_view #img_big').append(
              ...
              $.each(product.images, function (i, src) {
                var smallSrc = src
                  .replace('.png', '_68x82_crop_center.png')
                  .replace('.jpg', '_68x82_crop_center.jpg');
                var bigSrc = src
                  .replace('.png', '_377x380_crop_center.png')
                  .replace('.jpg', '_377x380_crop_center.jpg');
                $('#img_gallery .swiper-wrapper').append(
              ...
              var selectCallback = function (variant, selector) {
                if (variant && variant.available) {
                  jQuery('#quick_view__add')
                    .removeAttr('disabled')
                    .removeClass('disabled');

                  // VARIANT PRICES
                  if (variant.price < variant.compare_at_price) {
                    jQuery('#quick_view__price').html(
                      '<span class="money item_price">' +
                        Shopify.formatMoney(variant.price, '') +
                        '</span><span class="money compare-at-price money_sale">' +
                        Shopify.formatMoney(variant.compare_at_price, '') +
                        '</span><span class="money_sale_percent">Save' +
                        parseInt(
                          100 - (variant.price * 100) / variant.compare_at_price
                        ) +
                        '%</span>'
                    );
                  } else {
                  ...
                  if (variant.inventory_management != null) {
                    jQuery('#quick_view__availability span')
                ...
                for (var i = 0; i < selector.product.options.length; i++) {
                  if (
                    selector.product.options[i].name.toLowerCase() == 'color'
                  ) {
                    var selectorNum = i;
                  ...
                  if (
                    selector.product.options[i].name.toLowerCase() == 'size'
                  ) {
                    var selectorNum = i;
                    var selectorName = selector.product.options[i].name;
                    ...
                    function (newImageSizedSrc, newImage, element) {
                      ...
                      quickViewGallery.slides.each(function (i) {
                        var thumb = $(this)
                          .find('img')
                          .attr('src')
                          .replace('_68x82_crop_top', '')
                          .replace('_68x82_crop_center', '')
                          .replace('_68x82_crop_bottom', '')
                          .replace(/\?v=.*/, '');
                        var newImg = newImageSizedSrc
                          .replace('_377x380', '')
                          .replace(/\?v=.*/, '');
              ...
              $.fancybox($('#product_quick_view'), {
                ...
                tpl: {
                  ...
                  closeBtn:
                    '<a title="Close" id="quick_view__close" class="fancybox-item fancybox-close" href="javascript:;"></a>',
                },
                afterClose: function () {
        ...
        function renderColorOptions(num, name) {
          var colorSelect = $('#product_quick_view .single-option-selector').eq(
            num
          );
        ...
        function renderSizeOptions(num, name) {
          var sizeSelect = $('#product_quick_view .single-option-selector').eq(
            num
          );
  ...
  function ajaxCartRender() {
    ...
    jQuery.getJSON('/cart.js', function (data) {
      ...
      if (data.items.length == 0) {
        newHtml +=
          '<p class="alert alert-warning">' + theme.cartAjaxTextEmpty + '</p>';
      } else {
        data.items.forEach(function (item, i) {
          var image_url = item.image
            .replace('.png', '_140x140_crop_center.png')
            .replace('.jpg', '_140x140_crop_center.jpg');
          newHtml +=
            '<li class="cart_items"><img class="item_img" src="' +
            image_url +
            '"  alt="' +
            item.title +
            '" /><div class="item_desc"><a class="product_title" href="' +
            item.url +
            '">' +
            item.title.slice(0, 50) +
            '</a><span class="money">' +
            Shopify.formatMoney(item.price, theme.moneyFormat) +
            '</span><p class="product_quantity">Quantity: ' +
            item.quantity +
            '</p><a class="item_remove_btn" href="#" item-id="' +
            item.id +
            '"><svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M7 0H13V2H18V4H2V2H7V0Z"/><path fill-rule="evenodd" clip-rule="evenodd" d="M4 20H16V6H4V20ZM14 18H6V8H14V18Z"/></svg></a></div></li>';
        });

        newHtml +=
          '<div class="box_footer"><p class="cart_total"><b>' +
          theme.cartAjaxTextTotalPrice +
          ': </b><span class="money">' +
          Shopify.formatMoney(data.total_price, theme.moneyFormat) +
          '</span></p><a id="clear_cart_all_items" class="cart_clear" href="/cart/clear"><i class="fa fa-refresh" title="' +
          theme.cartAjaxTextClearCart +
          '"></i></a><a class="btn btn_inverted" href="/checkout">Checkout</a><a class="btn" href="/cart">' +
          theme.cartAjaxTextGoCart +
          '</a></div>';
      }
  ...
  jQuery(document).ready(function (t) {
    ...
    t(document).on('submit', 'form[action*="/cart/add"]', function (e) {
      e.preventDefault(),
        t(e.target).find('.btn-cart').attr('disabled', !0).addClass('disabled'),
        Shopify.addItemFromForm(e.target);
    }),
      t(document).on('click', '.btn-cart', function () {
        t.fancybox.showLoading(),
          t.fancybox.helpers.overlay.open({ parent: t('body') });
      }),
      t(e.FORM_UPDATE_CART_BUTTON).click(function (a) {
        a.preventDefault(),
          t(a.target.form)
            .find(e.FORM_UPDATE_CART_BUTTONS)
            .attr('disabled', !0)
            .addClass('disabled'),
          Shopify.updateCartFromForm(a.target.form);
      }),
      ...
      (Shopify.onItemAdded = function (e, a) {
        t(a).find('.btn-cart').attr('disabled', !1).removeClass('disabled'),
          Shopify.getCart();
      }),
      (Shopify.onCartUpdate = function (i, n) {
        ...
        t(e.TOTAL_PRICE).html(r),
          t(e.EMPTY_CART_MESSAGE).length > 0 &&
            0 == i.item_count &&
            (t(e.FORM_UPDATE_CART).hide(), t(e.EMPTY_CART_MESSAGE).show()),
          (n = n || !1),
          n &&
            i.item_count > 0 &&
            (t.each(i.items, function (i, n) {
              t(e.LINE_ITEM_PRICE_PREFIX + n.id).html(a(n.line_price)),
                t(e.LINE_ITEM_QUANTITY_PREFIX + n.id).val(n.quantity);
            }),
            t(n).find('input[value=0]').parents(e.LINE_ITEM_ROW).remove(),
            t(n)
              .find(e.FORM_UPDATE_CART_BUTTONS)
              .attr('disabled', !1)
              .removeClass('disabled'));
    ...
    (Shopify.addItemFromForm = function (t, o) {
      var r = {
        ...
        success: function (r) {
          ...
          $('#cart_added__price').html(
            Shopify.formatMoney(r.price, theme.moneyFormat)
          );
          $('#cart_added__total_quantity span').html(r.quantity);
          $('#cart_added__quantity span').html(r.quantity);
          $('#cart_added__total_price span').html(
            Shopify.formatMoney(r.final_line_price, theme.moneyFormat)
          );
          ...
          if (r.image) {
            $('#cart_added__img img')
              .attr(
                'src',
                r.image
                  .replace('.jpg', '_140x190_crop_center.jpg')
                  .replace('.png', '_140x190_crop_center.png')
              )
              .load(function () {
        ...
        error: function (t, o) {
          ...
          $('body').append(
            '<div id="cart_added" class="cart_error"><h4></h4><p class="alert alert-error"></p></div>'
          );
    ...
    (Shopify.updateCartAttributes = function (t, o) {
      ...
      jQuery.isArray(t)
        ? jQuery.each(t, function (t, o) {
            ...
            '' !== e &&
              (r +=
                'attributes[' + e + ']=' + attributeToString(o.value) + '&');
          })
        : 'object' == typeof t &&
          null !== t &&
          jQuery.each(t, function (t, o) {
